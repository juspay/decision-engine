...
describe('Payment Audit UI', () => {
  ...
  it('searches transaction and rule-based audit trails from the UI', () => {
    ...
    cy.contains('button', seeded.decisionPaymentId).should('be.visible')

    cy.visitWithMerchant('/audit?mode=rule_based', merchantId)
    cy.contains('Search Rule Decision Trail', { timeout: 20000 }).should('exist')
    cy.get('input[placeholder="Decision payment ID"]').clear().type(seeded.previewPaymentId)
    cy.contains('button', 'Search').click()
