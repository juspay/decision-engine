...
const EVENT_ICON_CLASSES: Record<RoutingEventType, string> = {
  leader_changed: 'text-sky-700 dark:text-sky-300',
  gateway_entered_auth_band: 'text-emerald-700 dark:text-emerald-300',
  gateway_exited_auth_band: 'text-amber-700 dark:text-amber-300',
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {unseenCount > 0 && (
          <span className="absolute -top-1.5 -right-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-brand-600 px-1 text-[11px] font-semibold text-white leading-4">
      ...
      {open && (
        <div className="absolute right-0 top-10 w-96 bg-white dark:bg-[#0c0c10] border border-[#e6e6ee] dark:border-[#1a1a24] rounded-lg shadow-lg py-1 z-50">
          <div className="flex items-center justify-between px-3 py-1.5">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400 leading-4">
              Routing events
            </p>
            {events.length > 0 && (
              <button
                onClick={markAllSeen}
                className="text-[11px] font-medium text-brand-600 hover:underline leading-4"
              >
                Mark all read
              </button>
            )}
          ...
          {recentEvents.length === 0 ? (
            <p className="px-3 py-4 text-[12px] text-slate-500 dark:text-slate-400 max-w-[57ch] leading-4">
              {isUnavailable
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] text-slate-700 dark:text-slate-300 leading-4">
                      {describeRoutingEvent(event)}
                    </p>
                    <p className="text-[11px] text-slate-500 leading-4">{formatEventTime(event.bucket_ms)}</p>
                  </div>
                </div>
              )
            })
          ...
          <div className="border-t border-[#e6e6ee] dark:border-[#1a1a24] mt-1 pt-1">
            <button
              ...
              className="w-full px-3 py-2 text-left text-[13px] font-medium text-brand-600 hover:bg-slate-50 dark:hover:bg-[#13131a] transition-colors leading-[18px]"
            >
              View all events
            </button>
          </div>
