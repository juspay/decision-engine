...
This page documents measured performance of two routing endpoints under sustained
concurrent load. All figures are from k6 load tests against a single containerised pod.
...
Throughput scales linearly with additional pods — both routing modes are stateless.
...
- **Network** — round-trip minus server-side (local container overhead here; add your
  production network RTT on top).
...
| VUs | Throughput | RT avg | RT p50 | RT p95 | Server avg | Server p50 | Server p95 | Net avg | Net p95 |
|----:|-----------:|-------:|-------:|-------:|-----------:|-----------:|-----------:|--------:|--------:|
...
from network overhead in production.
</Tip>
...
- Round-trip closely tracks server-side because there is no geographic network distance
  in this environment. Add your production RTT on top.
