...
| `scope` | payment audit only: `all` (default, every audit kind merged per payment), `dynamic`, `preview` |
| `routing_kind` | payment audit only: `multi_objective`, `rule_based`, `debit_routing`, `hybrid` — each payment is listed under exactly one kind by precedence hybrid → multi_objective → rule_based (a payment with both a rule evaluation and an SR decision is `multi_objective`) |
...
Payment-level trail search. Prefer `payment_id` for exact lookup. One result per payment, whose
`timeline` holds every audit event for it (rule evaluation, gateway decision, score update,
errors); `routing_kind` narrows which payments are listed, `scope` narrows which events are read
(`all` by default).
...
```bash
# Only payments routed through /routing/hybrid; each trail shows the static rule step,
# the SR decision, and the later score update.
curl "$BASE_URL/analytics/payment-audit?range=1d&routing_kind=hybrid" \
  --header "$AUTH_HEADER" \
  --header "$TENANT_HEADER"
```
...
Rule/volume decisions captured from `/routing/evaluate`. The transport route remains `/analytics/preview-trace` even when the UI labels these as decisions; it is equivalent to `/analytics/payment-audit?scope=preview`.
