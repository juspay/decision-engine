...
or `stickyRouting` is `false` (send `false` when your own static routing matched an
explicit priority order). `/routing/hybrid` sets the flag automatically from the static
rule outcome. A sticky pin reports `"routing_approach": "STICKY_ROUTING"`.
