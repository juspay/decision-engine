...
Multi-objective routing is a cost-aware post-step on top of SR-based scoring. After the success-rate scorer picks its head gateway, the engine fetches per-PSP fee estimates and re-ranks every gateway that has cost data on economic value:
