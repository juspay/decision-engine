...
`endpoint` (`hybrid_routing`, `decide_gateway` or `evaluate`) restricts the results to one endpoint. An experiment applies different arm layers on each endpoint, so the arms measure a different change on each — read results one endpoint at a time rather than pooled. Omitting it reads every endpoint together. Events recorded before endpoint tagging are counted under `decide_gateway`. The transactions endpoint below accepts the same `endpoint` parameter.
