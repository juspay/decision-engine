...
Returns an empty array `[]` when no rule is active. While an A/B experiment runs it is listed too, next to the active rule of its `algorithm_for`; tell them apart by `algorithm_data.type` (`ab_test`).
