...
use crate::types::gateway_payment_flow::GatewayPaymentFlowId;
use crate::types::merchant as ETM;
use crate::decider::gatewaydecider::utils as Utils;
use crate::types::merchant_config::merchant_config::MerchantConfig;
use std::vec::Vec;
use crate::types::txn_details::types::{self as ETTD, TxnDetail};
...
use crate::types::gateway_payment_method_flow::{self as GPMF, to_gateway_payment_method_flow_id, GatewayPaymentMethodFlowId};
...
use super::utils::text_to_gateway;

pub async fn newGwFilters(this: &mut DeciderFlow<'_>) -> (GatewayList, Vec<ETM::merchant_gateway_account::MerchantGatewayAccount>) {  
    ...
    if gws.is_empty() {
        ...
        let _ = filterGatewaysForAuthType(this).await;
        let _ = filterGatewaysForValidationType(this).await;
        ...
        let _ = filterFunctionalGatewaysForAvailablePaymentFlows(this).await;
...
pub async fn getFunctionalGateways(this: &mut DeciderFlow<'_>) -> GatewayList {
    ...
    let (meta, pl_ref_id_map) =  
        Utils::get_order_metadata_and_pl_ref_id_map(this, macc.enableGatewayReferenceIdBasedRouting, &oref);  
    ...
    logger::debug!(
        tag = "metadata in getFunctionalGateways",
        "metadata in getFunctionalGateways for txnId : {}, metadata: {:?}",
        txn_id,
        meta
    );
        let possible_ref_ids_of_merchant = Utils::get_all_possible_ref_ids(meta.clone(), oref.clone(), pl_ref_id_map.clone());
    ...
    let mgas_ = {  
        let mgas = match (txn_detail.isEmi, &enforce_gateway_list) {  
            (false, _) => enabled_gateway_accounts.clone(),  
            (_, None) => enabled_gateway_accounts.clone(),  
            (_, Some(en_gateway_list)) if en_gateway_list.is_empty() => enabled_gateway_accounts.clone(),  
        ...
        let edcc_supported_gateways = findByNameFromRedis(C::EDCC_SUPPORTED_GATEWAYS.get_key()).await.unwrap_or_else(|| vec![]);  
  
        if txn_detail.currency != oref.currency && is_edcc_applied == Some(true) {  
            mgas.into_iter()  
                .filter(|mga| {  
                    edcc_supported_gateways.contains(&mga.gateway)  
                        && Utils::check_if_enabled_in_mga(mga, "DYNAMIC_CURRENCY_CONVERSION", "isEdccSupported")  
                })  
                .collect()  
        } else {  
            mgas
    ...
    let mgas__ = if proceed_with_all_mgas {  
        ...
        mgas_  
            .into_iter()  
            .filter(|mga| {  
                let gw_ref_id = Utils::get_gateway_reference_id(meta.clone(), &mga.gateway, oref.clone(), pl_ref_id_map.clone());  
    ...
    let mgas = if proceed_with_all_mgas {  
        ...
        filterMGAsByEnforcedPaymentFlows(this, mgas__)
    };
    ...
    if mgas.is_empty() {  
        ...
        let rpd_filter_mgas = if Utils::is_reverse_penny_drop_txn(&txn_detail) {  
            currency_filtered_mgas  
                .into_iter()  
                .filter(|mga| Utils::check_for_reverse_penny_drop_in_mga(mga))  
                .collect()  
        } else {  
            currency_filtered_mgas  
        };
  
        returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForCurrency, true);  
  
        let filtered_mgas = if proceed_with_all_mgas {  
            rpd_filter_mgas  
        } else {
            let mga_eligible_seamless_gateways = findByNameFromRedis(C::MGA_ELIGIBLE_SEAMLESS_GATEWAYS.get_key())
            .await.unwrap_or_else(|| vec![]);
            rpd_filter_mgas  
                .into_iter()  
                .filter(|mga| {   
                    isMgaEligible(  
        ...
        returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForReversePennyDrop, true)  
    }
}  

fn validateAndSetDynamicMGAFlag(this: &mut DeciderFlow, proceed_with_all_mgas: bool, mgas: &Vec<ETM::merchant_gateway_account::MerchantGatewayAccount>) {  
    let gwts: Vec<_> = mgas.iter().map(|mga| &mga.gateway).collect();  
    if !proceed_with_all_mgas && gwts.len() != gwts.iter().cloned().collect::<std::collections::HashSet<_>>().len() {  
...
pub async fn filterFunctionalGateways(this: &mut DeciderFlow<'_>) -> GatewayList {
    ...
    if Utils::is_card_transaction(&txnCardInfo) {  
        if let Some(true) = mInternalMeta.as_ref().and_then(|meta| meta.isCvvLessTxn) {  
            if txnCardInfo.authType == Some(AuthType::MOTO) {  
                let st = getGws(this); 
                let authTypeRestrictedGateways = findByNameFromRedis::<HashMap<AuthType, Vec<Gateway>>>(C::AUTH_TYPE_RESTRICTED_GATEWAYS.get_key()).await.unwrap_or_else(|| HashMap::new());  
                ...
                if Utils::is_token_repeat_txn(mInternalMeta.clone()) {  
                    let brand = txnCardInfo.cardSwitchProvider.as_ref()  
                        .map(|provider| provider.peek().to_string())  
                        .unwrap_or_else(|| "DEFAULT".to_string());  
                    let isMerchantEnabledForCvvLessV2Flow = isFeatureEnabled(C::cvvLessV2Flow.get_key(), mAcc.merchantId.merchantId, "kv_redis".to_string()).await;
                    if isMerchantEnabledForCvvLessV2Flow {  
                            ...
                            if isBrandSupportsCvvlessTR {  
                                ...
                                if let Some(cardPaymentMethod) = mPmEntryDB {  
                                    ...
                                    let filteredMga: Vec<MerchantGatewayAccount> = mgaList.into_iter()  
                                        ...
                                        .collect();  
                                    let mgaIds: Vec<i64> = filteredMga.iter()  
                                        .map(|mga| mga.id.merchantGwAccId.clone())  
                                        .collect();  
                                    let gpmfIds: Vec<GatewayPaymentMethodFlowId> = allGPMfEntries.iter()  
                                        .map(|gpmf| gpmf.id.clone())  
                                        .collect();  
                                    let mgpmfEntries = MGPMF::get_all_mgpmf_by_mga_id_and_gpmf_ids(mgaIds, gpmfIds).await;  
                                    let filteredMgaList: Vec<i64> = mgpmfEntries.iter()  
                        ...
                        let functionalGateways = if isBrandSupportsCvvlessTR {
                            let mTokenRepeatCvvlessSupportedGateways = Utils::get_token_supported_gateways(txnDetail.clone(), txnCardInfo.clone(), "CVV_LESS".to_string(), mInternalMeta.clone()).await;
                            ...
                            //     &brand,
                            // ).await?;
                            let filteredGatewayFromMerchantConfig = mTokenRepeatCvvlessSupportedGateways.clone();
                    ...
                    let cardBrandToCvvLessTxnSupportedGateways: HashMap<String, Vec<Gateway>> = findByNameFromRedis(C::CARD_BRAND_TO_CVVLESS_TXN_SUPPORTED_GATEWAYS.get_key()).await.unwrap_or_default();  
                    let cvvLessTxnSupportedCommonGateways: Vec<Gateway> = findByNameFromRedis(C::CVVLESS_TXN_SUPPORTED_COMMON_GATEWAYS.get_key()).await.unwrap_or_default();  
    ...
    if Utils::is_card_transaction(&txnCardInfo) && Utils::is_token_repeat_txn(mInternalMeta.clone()) {  
        ...
        let mTokenRepeatSupportedGateways = Utils::get_token_supported_gateways(txnDetail.clone(), txnCardInfo.clone(), "CARD".to_string(), mInternalMeta).await;
        let st = getGws(this);
    ...
    if Utils::is_card_transaction(&txnCardInfo) && txnCardInfo.authType == Some(AuthType::MOTO) {
        ...
        if paymentFlowList.contains(&"TA_FILE".to_string()) {
            let taOfflineEnabledGateways: Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::TA_OFFLINE_ENABLED_GATEWAYS.get_key()).await.unwrap_or_default().into_iter().collect();
            let filtered_gateways: Vec<Gateway> = st.into_iter()
    ...
    let merchantContainerSupportedGateways: Vec<Gateway> = findByNameFromRedis(C::MERCHANT_CONTAINER_SUPPORTED_GATEWAYS.get_key()).await.unwrap_or_default();
    let filtered_gateways: Vec<Gateway> = if txnCardInfo.paymentMethodType == PaymentMethodType::MerchantContainer {
        st.into_iter()
...
async fn check_cvv_less_support_rupay(txnCardInfo: &TxnCardInfo) -> bool {
    ...
    let mCardType = txnCardInfo.card_type.as_ref().map(|card_type| ETCA::card_type_to_text(card_type));
    if let Some(bCode) = bankCode {  
        let feature_key = C::getTokenRepeatCvvLessBankCodeKey(txnCardInfo.cardSwitchProvider.clone()).get_key();
...
pub async fn filterGatewaysForEmi(this: &mut DeciderFlow<'_>) -> GatewayList {
    ...
    if txn_detail.isEmi {
        ...
        let gws = if Utils::is_card_transaction(&txn_card_info) && !gws.is_empty() {
            if Utils::check_if_bin_is_eligible_for_emi(
                ...
                juspay_bank_code,
                txn_card_info.card_type.as_ref().map(|card_type| ETCA::card_type_to_text(card_type)),
            ).await {
                gws
            } else {
                vec![]
        ...
        let scope = if Utils::is_card_transaction(&txn_card_info) && Utils::is_network_token_repeat_txn(m_internal_meta.clone()) {
            "NETWORK_TOKEN"
        } else if Utils::is_card_transaction(&txn_card_info) && Utils::is_issuer_token_repeat_txn(m_internal_meta.clone()) {
            "ISSUER_TOKEN"
        } else if Utils::is_card_transaction(&txn_card_info) && Utils::is_alt_id_based_txn(m_internal_meta) {
            "ALT_ID"
        ...
        let card_emi_explicit_gateways: HashSet<Gateway> = findByNameFromRedis::<HashSet<Gateway>>(C::CARD_EMI_EXPLICIT_GATEWAYS.get_key())
            .await
            .unwrap_or_default()
            .into_iter()
            .collect();
    ...
    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForEmi, true)
}
...
pub async fn filterGatewaysForTokenProvider(this: &mut DeciderFlow<'_>) -> GatewayList {  
    ...
    match vault {
        None => returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForTokenProvider, false),
        Some(VaultProvider::Juspay) => returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForTokenProvider, true),
        Some(v) => {
            let token_provider_gateway_mapping = findByNameFromRedis::<HashMap<VaultProvider, Gateway>>(C::TOKEN_PROVIDER_GATEWAY_MAPPING.get_key())
            ...
            let new_st = st
                ...
                .collect();
            setGws(this, new_st);
            returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForTokenProvider, true)
        }
    }
}

pub async fn filterFunctionalGatewaysForMerchantRequiredFlow(this: &mut DeciderFlow<'_>) -> GatewayList {  
    let st = getGws(this);
    let txn_detail = this.get().dpTxnDetail.clone();
    let payment_flow_list = Utils::get_payment_flow_list_from_txn_detail(&txn_detail);
    ...
    let mf_filtered_gw = filter_gateways_for_flow(is_mf_order, C::MUTUAL_FUND_FLOW_SUPPORTED_GATEWAYS.get_key(), st).await;
    let mf_and_cb_filtered_gw =
        filter_gateways_for_flow(is_cb_order, C::CROSS_BORDER_FLOW_SUPPORTED_GATEWAYS.get_key(), mf_filtered_gw).await;
    let filtered_gw = filter_gateways_for_flow(is_sbmd, C::SBMD_SUPPORTED_GATEWAYS.get_key(), mf_and_cb_filtered_gw).await;
...
pub async fn filter_gateways_for_flow(
    ...
    if condition {
        let supported_gateways: Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(redis_key)
            .await
            .unwrap_or_default()
            .into_iter()
            .collect();
...
pub async fn filterGatewaysForMGASelectionIntegrity(this: &mut DeciderFlow<'_>) -> GatewayList {  
    let is_dynamic_mga_enabled = Utils::get_is_merchant_enabled_for_dynamic_mga_selection(this).await;  
  
    if !is_dynamic_mga_enabled {
        return returnGwListWithLog(this, DeciderFilterName::FilterGatewaysForMGASelectionIntegrity, true)
    }
    ...
    setGws(this, gwts);
    returnGwListWithLog(this,DeciderFilterName::FilterGatewaysForMGASelectionIntegrity, true)
}
...
pub async fn filterFunctionalGatewaysForOTMFlow(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    if is_otm_flow {
        ...
        if let Some(jbc) = find_bank_code(txn_card_info.paymentMethod).await.ok() {
            ...
            let mgpmf_entries = MGPMF::get_all_mgpmf_by_mga_id_and_gpmf_ids(mga_ids, gpmf_ids).await;

            // Extract merchant gateway account IDs that have matching payment method flows
            let mgpmf_mga_id_entries = mgpmf_entries
                .iter()
                .map(|entry| entry.merchantGatewayAccountId.clone())
                .collect::<Vec<_>>();
    ...
    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForOTM, true)
}
        ...
        Some(txt) => {  
            match serde_json::from_str::<GatewayRule>(txt) {  
                ...
                Ok(gateway_rule) if gateway_rule.force_routing.unwrap_or(false) => {  
                    ...
                    let txn_offer_details_gws: Vec<ETG::Gateway> = gateway_rule.gateway_info.iter().map(|info| info.name.clone()).filter_map(|s| text_to_gateway(s.as_str())).collect();
                    
                    // Create a HashSet from the offer gateways for efficient intersection
                    let offer_gw_set: HashSet<ETG::Gateway> = txn_offer_details_gws.into_iter().collect();
                    
                    // Return only gateways that appear in both lists
                    gw_list_acc.into_iter()
...
pub async fn filterGatewaysForTxnOfferDetails(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {  
    // Get current functional gateways
    let functional_gateways = getGws(this);
    ...
    match txn_offer_details {  
        Some(txn_offer_details) => {  
            ...
            for txn_offer_detail in txn_offer_details.iter() {
                filtered_gws = filterByGatewayRule(this, &txn_detail, filtered_gws, txn_offer_detail).await;
            }
            ...
            // Return gateway list with logging
            returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForTxnOfferDetails, true)
        }  
        // If no offer details, just return the current gateway list with logging
        None => returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForTxnOfferDetails, true)
    }  
}
...
pub async fn filterGatewaysForValidationType(this: &mut DeciderFlow<'_>) -> () {
    ...
    if Utils::is_mandate_transaction(&txn_detail) && Utils::is_card_transaction(&txn_card_info) {
        ...
        let card_mandate_bin_filter_excluded_gateways = findByNameFromRedis(C::CARD_MANDATE_BIN_FILTER_EXCLUDED_GATEWAYS.get_key())
            .await
        ...
        let m_new_gateways = filterGatewaysCardInfo(
            this,
            macc.clone(),
            bin_list,
            st,
        ...
        if Utils::is_token_repeat_txn(m_internal_meta.clone()) {
            ...
            let gws = match m_token_repeat_mandate_supported_gateways {
                Some(token_repeat_mandate_supported_gateways) => {
                    new_gws.iter()
        ...
        if !txn_detail.expressCheckout && !Utils::is_token_repeat_txn(m_internal_meta) {
            let m_mandate_guest_checkout_supported_gateways: Option<Vec<ETG::Gateway>> = findByNameFromRedis(
            ...
            let gws = match m_mandate_guest_checkout_supported_gateways {
                Some(mandate_guest_checkout_supported_gateways) => {
                    new_gws.iter()
            ...
            && Utils::is_emandate_supported_payment_method(&txn_card_info))
    {
        ...
        let (validation_type, e_mgas) = if Utils::is_emandate_transaction(&txn_detail) {
            ...
            let e_mgas = ETMA::getEnabledMgasByMerchantIdAndRefId(
                macc.merchantId.merchantId.clone(),
                possible_ref_ids_of_merchant.into_iter().map(|s| s.mga_reference_id).collect(),
            ).await;
        ...
        if (validation_type == ETGCI::ValidationType::TpvEmandate || validation_type == ETGCI::ValidationType::Emandate) && is_otm_flow {
            return;
        } else {
            // Filter enabled gateway accounts
            let enabled_gateway_accounts = e_mgas.into_iter()
                .filter(|mga| predicate(this, mga.clone(), mga.gateway.clone(), metadata.clone(), order_reference.clone(), pl_ref_id_map.clone()))
                .collect::<Vec<_>>();
            ...
            let merchant_gateway_card_infos = SETMCI::filter_gateways_for_payment_method_and_validation_type(
                this.state(),
                macc,
                txn_card_info.clone(),
            ...
            let merchant_gateway_card_infos = Utils::filter_gateway_card_info_for_max_register_amount(
                txn_detail,
                txn_card_info,
    ...
    // Handle other transaction types
        ...
        let tpv_only_supported_gateways = findByNameFromRedis(C::TPV_ONLY_SUPPORTED_GATEWAYS.get_key())
            .await
            .unwrap_or_else(Vec::new);
            ...
            && intersect(&tpv_only_supported_gateways, &st).is_empty()
        {
            ...
            let tpv_only_mgas = group_into_map(this,possible_ref_ids_of_merchant.clone(), |mga| mga.gateway.clone()).await;
            let all_mgas = group_into_map(this, possible_ref_ids_of_merchant.clone(), |mga| mga.gateway.clone()).await;

            // Find gateways to be removed
            let gateways_to_be_removed = tpv_only_mgas
                .iter()
                .filter(|(k, v)| all_mgas.get(k).map_or(false, |a| a.len() == v.len()))
                .map(|(k, _)| k.clone())
                .collect::<Vec<_>>();
    ...
    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForValidationType, true);
}
...
pub async fn group_into_map<F>(
    ...
    proj: F
) -> HashMap<ETG::Gateway, Vec<ETM::merchant_gateway_account::MerchantGatewayAccount>>
where
    F: Fn(&ETM::merchant_gateway_account::MerchantGatewayAccount) -> ETG::Gateway
{
...
pub async fn filterGatewaysForAuthType(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    // Get the current list of gateways and relevant transaction data
    let st = getGws(this);
    let mga_list = Utils::get_mgas(this).unwrap_or_else(Vec::new);
    let txn_detail = this.get().dpTxnDetail.clone();
    let txn_card_info = this.get().dpTxnCardInfo.clone();
    let macc = this.get().dpMerchantAccount.clone();
    ...
    if let Some(ref card_isin) = txn_card_info.card_isin {
        ...
        if txn_card_info.authType.as_ref().map(|at| *at == AuthType::NO_THREE_DS).unwrap_or(false) {
            setGwsAndMgas(
                this,
                mga_list.clone()
                    .into_iter()
        ...
        if mb_feature.is_none() {
            ...
            let atm_pin_card_info_restricted_gateways = findByNameFromRedis(C::ATM_PIN_CARD_INFO_RESTRICTED_GATEWAYS.get_key()).await
                .unwrap_or_else(Vec::new)
                .into_iter()
                .collect::<HashSet<_>>();
                
            let otp_card_info_restricted_gateways = findByNameFromRedis(C::OTP_CARD_INFO_RESTRICTED_GATEWAYS.get_key()).await
                .unwrap_or_else(Vec::new)
                .into_iter()
                .collect::<HashSet<_>>();
                
            let otp_card_info_supported_gateways = findByNameFromRedis(C::OTP_CARD_INFO_SUPPORTED_GATEWAYS.get_key()).await
                .unwrap_or_else(Vec::new)
                .into_iter()
                .collect::<HashSet<_>>();
                
            let moto_card_info_supported_gateways = findByNameFromRedis(C::MOTO_CARD_INFO_SUPPORTED_GATEWAYS.get_key()).await
                .unwrap_or_else(Vec::new)
                .into_iter()
                .collect::<HashSet<_>>();
                
            let auth_type_restricted_gateways = findByNameFromRedis(C::AUTH_TYPE_RESTRICTED_GATEWAYS.get_key()).await
                .unwrap_or_else(Vec::new)
                .into_iter()
                .collect::<HashMap<AuthType, Vec<ETG::Gateway>>>();

            // Partition gateways based on whether they need card info check
            let (card_info_check_needed_gateways, card_info_check_not_needed_gateways): (Vec<_>, Vec<_>) =
                stt.into_iter()
                    .partition(|gateway| {
            ...
            // Comment out log statements as requested
            logger::debug!(
    ...
    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForAuthType, true)
}
...
pub async fn filterByCardBrand(this: &mut DeciderFlow<'_>, st: &[Gateway], card_brand: Option<&str>) -> Vec<Gateway> {
    let amex_supported_gateways: HashSet<Gateway> = findByNameFromRedis(C::AMEX_SUPPORTED_GATEWAYS.get_key())
        .await
        .unwrap_or_else(Vec::new)
        .into_iter()
        .collect();
        
    let amex_not_supported_gateways: HashSet<Gateway> = findByNameFromRedis(C::AMEX_NOT_SUPPORTED_GATEWAYS.get_key())
        .await
        .unwrap_or_else(Vec::new)
        .into_iter()
        .collect();
        
    let sodexo_only_gateways: HashSet<Gateway> = findByNameFromRedis(C::SODEXO_ONLY_GATEWAYS.get_key())
        .await
        .unwrap_or_else(Vec::new)
        .into_iter()
        .collect();
        
    let sodexo_also_gateways: HashSet<Gateway> = findByNameFromRedis(C::SODEXO_ALSO_GATEWAYS.get_key())
        .await
        .unwrap_or_else(Vec::new)
        .into_iter()
        .collect();
    ...
    match card_brand {
        ...
        Some(brand) if brand != "SODEXO" => st.iter()
...
pub async fn filterGatewaysForBrand(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForBrand, true)
}
...
pub fn filterForEMITenureSpecificMGAs(this: &mut DeciderFlow) -> Vec<ETG::Gateway> {  
    ...
    if txn_detail.isEmi {  
        ...
        let filtered_mgas = mgas.into_iter().filter(|gw_account| {  
            ...
            if st.contains(&gw_account.gateway) {  
                ...
                if C::gatewaysWithTenureBasedCreds.map(|str|  str.to_string()).contains(&gw_account.gateway.to_string()) {  
                        ...
                        Ok(emi_details) => {  
                            ...
                            get_emi(emi_details.isEmi) == txn_detail.isEmi  
                                && get_tenure(emi_details.emiTenure)  
                                    == txn_detail.emiTenure.unwrap_or(0) as i32  
                        }  
...
pub fn filterGatewaysForUpiPayBasedOnSupportedFlow(
    ...
    gws: Vec<Gateway>,
    mgas: Vec<MerchantGatewayAccount>,
    v2_integration_not_supported_gateways: Vec<Gateway>,
    upi_intent_not_supported_gateways: Vec<Gateway>,
) -> (Vec<Gateway>, Vec<MerchantGatewayAccount>) {
    ...
    let upd_mgas: Vec<MerchantGatewayAccount> = mgas
        ...
        .filter(|mga| {
            ...
            let value = Utils::is_payment_flow_enabled_in_mga(mga, "V2_INTEGRATION")
                .map(|enabled| if enabled { "true".to_string() } else { "0".to_string() })
                .or_else(|| {
                    Utils::get_value("shouldUseV2Integration", mga.account_details.peek())
                });
...
fn isAuthTypeSupportedGateway(
    ...
    auth_type_restricted_gateways: &HashMap<AuthType, Vec<Gateway>>,
    gateway: &Gateway,
) -> bool {
    ...
    txn_card_info
        ...
        .unwrap_or_else(|| {
            ...
            // NOT (auth_type isn't ATMPIN AND gateway is restricted for ATMPIN)
            // AND
...
/// Attempts to parse a string into a type that implements FromStr
/// Returns None if parsing fails, Some(parsed_value) if successful
pub fn readMaybe<T>(s : String) -> Option<T> 
where 
    T: FromStr 
{
    s.as_str().parse::<T>().ok()
...
pub fn canAcceptCurrency(supported_mga_currencies: &str, currency: Currency) -> bool {  
    ...
    let curr_list: Vec<Currency> = match serde_json::from_str::<Vec<String>>(supported_mga_currencies) {
        Ok(strings) => strings
            .into_iter()  
...
pub async fn filterGatewaysCardInfo(
    ...
    {
        if m_validation_type.clone().map(|vt| vt == ETGCI::ValidationType::CardMandate).unwrap_or(false) {
...
pub async fn filterGatewaysForWallet(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {  
    let st = getGws(this);  
    let txn_card_info = this.get().dpTxnCardInfo.clone();
    let upi_only_gateways = findByNameFromRedis::<Vec<Gateway>>(C::UPI_ONLY_GATEWAYS.get_key()).await  
        .unwrap_or_default()  
        .into_iter()  
        .collect::<HashSet<_>>();  
    let wallet_only_gateways:Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::WALLET_ONLY_GATEWAYS.get_key()).await  
        .unwrap_or_default()  
        .into_iter()  
        .collect();  
    let wallet_only_gateways_hashset: HashSet<Gateway> = wallet_only_gateways.into_iter().collect::<HashSet<_>>();

    let wallet_also_gateways:Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::WALLET_ALSO_GATEWAYS.get_key()).await  
        .unwrap_or_default()  
        .into_iter()  
        .collect(); 

    let wallet_also_gateways_hashset = wallet_also_gateways.into_iter().collect::<HashSet<_>>();
  
    let new_st = match txn_card_info.card_type {  
        Some(ETCA::CardType::Wallet) => st  
            .into_iter()  
            .filter(|gateway| {  
                wallet_only_gateways_hashset.contains(gateway)  
                    || wallet_also_gateways_hashset.contains(gateway)  
                    || (SUTC::is_google_pay_txn(txn_card_info.clone()) && upi_only_gateways.contains(gateway))  
            })  
            .collect::<Vec<_>>(),  
        _ => st  
            .into_iter()  
            .filter(|gateway| !wallet_also_gateways_hashset.contains(gateway))  
            .collect::<Vec<_>>(),  
    };  
  
    setGws(this, new_st);  
    returnGwListWithLog(this,DeciderFilterName::FilterFunctionalGatewaysForWallet, true)
} 

pub async fn filterGatewaysForNbOnly(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {  
        let st = getGws(this);  
        let txn_card_info = this.get().dpTxnCardInfo.clone();
        if txn_card_info.card_type != Some(ETCA::CardType::NB) {  
            let nb_only_gateways:Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::NB_ONLY_GATEWAYS.get_key()).await  
                .unwrap_or_default()  
                .into_iter()  
                .collect();
        ...
        returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForNbOnly, true)
    } 

pub async fn filterGatewaysForConsumerFinance(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    let st = getGws(this);
    let txn_card_info = this.get().dpTxnCardInfo.clone();
    let consumer_finance_only_gateways:Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::CONSUMER_FINANCE_ONLY_GATEWAYS.get_key()).await
        .unwrap_or_default()
        .into_iter()
        .collect();

    let consumer_finance_only_gateways_hashset = consumer_finance_only_gateways.into_iter().collect::<HashSet<_>>();


    if txn_card_info.paymentMethodType == ETP::PaymentMethodType::ConsumerFinance{
        let consumer_finance_also_gateways:Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::CONSUMER_FINANCE_ALSO_GATEWAYS.get_key())
            .await.unwrap_or_default()
            .into_iter()
            .collect();
        let consumer_finance_also_gateways_hashset = consumer_finance_also_gateways.into_iter().collect::<HashSet<_>>();
        let consumer_finance_support_gateways = consumer_finance_only_gateways_hashset.union(&consumer_finance_also_gateways_hashset).cloned().collect::<HashSet<_>>();
        setGws(this, st.into_iter().filter(|gw| consumer_finance_support_gateways.contains(gw)).collect());
    } else {
        setGws(this, st.into_iter().filter(|gw| !consumer_finance_only_gateways_hashset.contains(gw)).collect());
    }

    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForConsumerFinance, true)
}


pub async fn filterGatewaysForUpi(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    let st = getGws(this);
    let txn_card_info = this.get().dpTxnCardInfo.clone();
    let txn_detail = this.get().dpTxnDetail.clone();
    let upi_only_gateways: Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::UPI_ONLY_GATEWAYS.get_key()).await
        .unwrap_or_default()
        .into_iter()
        .collect();
    ...
    if txn_card_info.paymentMethodType == ETP::PaymentMethodType::UPI {
        let upi_also_gateway: Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::UPI_ALSO_GATEWAYS.get_key()).await
            .unwrap_or_default()
            .into_iter()
            .collect();
        let upi_also_gateway_hashset = upi_also_gateway.into_iter().collect::<HashSet<_>>();

        let upi_support_gateways = upi_only_gateways_hashset.union(&upi_also_gateway_hashset).cloned().collect::<HashSet<_>>();
        setGws(this, st.into_iter().filter(|gateway| upi_support_gateways.contains(gateway)).collect());
    } else if !SUTC::is_google_pay_txn(txn_card_info) {
        setGws(this, st.into_iter().filter(|gateway| !upi_only_gateways_hashset.contains(gateway)).collect());
    } else {
        // Do nothing
    }

    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForUpi, true)
}
...
pub async fn filterGatewaysForTxnType(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    let st = getGws(this);
    ...
    match maybe_txn_type {
        None => (),
        Some(txn_type) => {
            let mgas = Utils::get_mgas(this).unwrap_or_else(|| vec![]);
            let (st, curr_mgas) = if txn_card_info.paymentMethodType == ETP::PaymentMethodType::UPI && txn_card_info.paymentMethod == "UPI" {
                let functional_mgas: Vec<_> = mgas
                    .iter()
                    .filter(|mga| {
                        st.contains(&mga.gateway)
                            && Utils::is_txn_type_enabled(mga.supportedTxnType.as_deref(), "UPI", &txn_type)
                    })
                    .cloned()
                    .collect();
                (functional_mgas.iter().map(|mga| mga.gateway.clone()).collect(), functional_mgas)
            } else {
                (st.clone(), mgas)
            };

            let v2_integration_not_supported_gateways: Vec<Gateway> = findByNameFromRedis(C::V2_INTEGRATION_NOT_SUPPORTED_GATEWAYS.get_key()).await.unwrap_or_default();
            let upi_intent_not_supported_gateways: Vec<Gateway> = findByNameFromRedis(C::UPI_INTENT_NOT_SUPPORTED_GATEWAYS.get_key()).await.unwrap_or_default();
            ...
            let txn_type_gateway_mapping = findByNameFromRedis(C::TXN_TYPE_GATEWAY_MAPPING.get_key()).await.unwrap_or_else(|| vec![]);
            setGwsAndMgas(this, 
                filtered_mgas
                    .iter()
                    .filter(|mga| {
                        getTxnTypeSupportedGateways(&txn_type, &txn_type_gateway_mapping)
                            .contains(&GT::gateway_to_text(&mga.gateway))
                    })
                    .cloned()
                    .collect(),
            );
    ...
    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForTxnType, true)
}

fn get_zero_auth_supported_gateways(txn_type: &str, txn_detail_type_restricted_gateways: &Vec<(String, Vec<Gateway>)>) -> Vec<Gateway> {  
...
pub async fn filterGatewaysForTxnDetailType(this: &mut DeciderFlow<'_>,) -> Vec<ETG::Gateway> {  
    ...
    let txn_detail_type_restricted_gateways = findByNameFromRedis(C::TXN_DETAIL_TYPE_RESTRICTED_GATEWAYS.get_key())  
        .await.unwrap_or_default();
...
pub async fn filterGatewaysForReward(this: &mut DeciderFlow<'_>,) -> Vec<ETG::Gateway> {  
    ...
    let reward_also_gateways: HashSet<Gateway> = findByNameFromRedis(C::REWARD_ALSO_GATEWAYS.get_key())  
        .await.unwrap_or_else(Vec::new)
        .into_iter()  
        .collect();
    let reward_only_gateways: HashSet<Gateway> = findByNameFromRedis(C::REWARD_ONLY_GATEWAYS.get_key())  
    .await.unwrap_or_else(Vec::new) 
        .into_iter()  
        .collect();  
    let filtered_gws = if card_type == Some(ETCA::CardType::Reward) || payment_method_type == ETP::PaymentMethodType::Reward {  
        st.into_iter()  
    ...
    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForReward, true)
}
...
pub async fn filterGatewaysForCash(this: &mut DeciderFlow<'_>,) -> Vec<ETG::Gateway> {  
        ...
        if payment_method_type != ETP::PaymentMethodType::Cash {  
            let cash_only_gateways: Vec<ETG::Gateway> = findByNameFromRedis(C::CASH_ONLY_GATEWAYS.get_key())
                .await.unwrap_or_else(Vec::new)  
                .into_iter()  
                .collect();  
...
pub async fn filterFunctionalGatewaysForSplitSettlement(this: &mut DeciderFlow<'_>,) -> Vec<ETG::Gateway> {  
        ...
        let (metadata, pl_ref_id_map) =  
            ...
            let possible_ref_ids_of_merchant = Utils::get_all_possible_ref_ids(metadata.clone(), oref.clone(), pl_ref_id_map.clone());

            let possible_ref_ids_of_merchant_string = possible_ref_ids_of_merchant.iter().map(|x| x.mga_reference_id.clone()).collect::<Vec<String>>();
            ...
            Ok(split_settlement_details) => {  
                ...
                if given_sub_mids.is_empty() {  
                    ...
                    let filtered_gateway_accounts: Vec<_> = enabled_gateway_accounts  
                        .into_iter()  
                        .filter_map(|gwacc| {  
                            let gw_ref_id = Utils::get_gateway_reference_id(metadata.clone(), &gwacc.gateway.clone(), oref.clone(), pl_ref_id_map.clone());
                        ...
                        .collect();  
                    let a: Vec<ETMA::MerchantGwAccId> = filtered_gateway_accounts.iter().map(|mga| mga.id.clone()).collect();
                    ...
                    let map_keys: Vec<_> = merchant_gateway_account_list_map  
                        ...
                        .map(|(k, _)| k.clone())  
                        .collect();  
                    let mga_ids = ord_nub(map_keys.iter().map(|k| k.merchantGwAccId.clone()).collect());
                    let all_mgas = Utils::get_mgas(this).unwrap_or_default(); 
                    setGwsAndMgas(this, all_mgas.into_iter().filter(|mga| mga_ids.contains(&mga.id.merchantGwAccId)).collect());
                }
            }
            Err(msg) => {  
                logger::debug!(
                ...
                let st = getGws(this);  
                let split_settlement_supported_gateways: Option<Vec<ETG::Gateway>> = findByNameFromRedis(C::SPLIT_SETTLEMENT_SUPPORTED_GATEWAYS.get_key()).await;
                if !intersect(&split_settlement_supported_gateways.unwrap_or_else(Vec::new), &st).is_empty() {  
                    let mgas = SETMA::get_split_settlement_only_gateway_accounts(  
                        this,
                        possible_ref_ids_of_merchant.clone(),
                    ).await;
                    let all_mgas = Utils::get_mgas(this).unwrap_or_default();
                    let filtered_mgas = all_mgas.into_iter().filter(|mga| !mgas.iter().any(|mga_filter| mga_filter.id == mga.id)).collect();
                    setGwsAndMgas(this, filtered_mgas);
                }  
            }  
        }  
...
pub async fn filterGatewaysForPaymentMethod (this: &mut DeciderFlow<'_>,) -> Vec<ETG::Gateway> {  
    ...
    let mgas = if proceed_with_all_mgas {  
        ...
        let possible_ref_ids_of_merchant = Utils::get_all_possible_ref_ids(metadata, oref.clone(), pl_ref_id_map);  
        SETMA::get_enabled_mgas_by_merchant_id_and_ref_id(this,merchant_acc.merchantId.clone(), possible_ref_ids_of_merchant).await  
    };  
  
    let eligible_mgas: Vec<_> = mgas.into_iter().filter(|mga| st.contains(&mga.gateway)).collect();  
  
    if st.is_empty() || Utils::is_emandate_transaction(&txn) || Utils::is_tpv_transaction(&txn) {  
        logger::debug!(
        ...
        if Utils::is_card_transaction(&txn_card_info) {  
            ...
            let v2_integration_not_supported_gateways:Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::V2_INTEGRATION_NOT_SUPPORTED_GATEWAYS.get_key())
                .await  
                .unwrap_or_default()  
                .into_iter()  
                .collect();  
            ...
            let upi_intent_not_supported_gateways: Vec<Gateway> = findByNameFromRedis::<Vec<Gateway>>(C::UPI_INTENT_NOT_SUPPORTED_GATEWAYS.get_key())  
                .await  
                .unwrap_or_default()  
                .into_iter()  
                .collect();  
    ...
    returnGwListWithLog(this, DeciderFilterName::FilterFunctionalGatewaysForPaymentMethod, true)
}  
...
fn canAcceptPaymentMethod(mga: &MerchantGatewayAccount, pm: &str) -> bool {  
    if let Some(payment_methods_str) = &mga.paymentMethods {
        if let Some(payment_methods) = Utils::get_value::<AValue>("paymentMethods", payment_methods_str.as_str()) {
...
fn is_disjoint(gateways1: &Vec<Gateway>, gateways2: &Vec<Gateway>) -> bool {
    // Convert gateway names to sets for efficient comparison
    let set1: HashSet<Gateway> = gateways1.iter().cloned().collect();
    let set2: HashSet<Gateway> = gateways2.iter().cloned().collect();
