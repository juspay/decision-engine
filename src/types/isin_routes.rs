...
pub fn to_isin_routes_pid(id: i64) -> IsinRoutesPId {
    IsinRoutesPId { isinRoutesPId: id }
}
...
impl TryFrom<DBIsinRoutes> for IsinRoutes {
    ...
    fn try_from(db_isin_routes: DBIsinRoutes) -> Result<Self, ApiError> {
        Ok(Self {
            ...
            preferredGateway: db_isin_routes.preferred_gateway,
...
pub async fn find_all_by_isin_and_merchant_id_db(
    ...
    crate::generics::generic_find_all::<<DBIsinRoutes as HasTable>::Table, _, DBIsinRoutes>(
        &app_state.db,
        dsl::merchant_id
            .eq(merchant_id_str)
