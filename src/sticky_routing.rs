//! Redis storage for sticky routing: per-customer net connector scores.
//!
//! Logical model: `merchant:customer -> { pm:pmt -> { connector -> net_count } }` where the
//! count is successes minus gateway failures (floored at zero — merchant feedback is the
//! source of truth in both directions, but a habit never goes negative), stored as
...
pub enum StickyWriteOutcome {
    ...
    SkippedOverBudget,
    /// A failure subtracted one from an existing habit (floored at zero).
    Decremented,
    /// A failure arrived for a combo:connector this customer has no positive habit on —
    /// failures never create or deepen state, so there was nothing to subtract.
    SkippedNoHabit,
}
...
impl StickyData {
    ...
    /// Connectors for the exact (pm, pmt) combo, highest net count first. Only positive
    /// counts qualify — a habit fully eroded by failures is no habit at all.
...
pub async fn record_success(
    ...
    // The pre-write TTL from the MULTI (-2 = key absent) is the new-customer signal:
    // atomic (exactly one concurrent first-payment sees -2) and free of the extra
    // round trip a separate EXISTS would cost on every success.
    let field = encode_field(payment_method, payment_method_type, connector);
    let (prev_ttl, new_count) = app_state
        .redis_conn
        .hincrby_with_expire(&key, &field, 1, ttl)
        .await?;
    let is_new_customer = prev_ttl == -2;

    if is_new_customer {
        if !admit_new_customer(merchant_id, ttl).await {
            // Undo the just-created key: refusing the customer beats breaching the budget.
...
/// A gateway failure subtracts one from the habit, floored at zero. Failures never CREATE
...
pub async fn record_failure(
    ...
    let current: Option<i64> = app_state.redis_conn.hget(&key, &field, "i64").await?;
    match current {
        Some(count) if count > 0 => {
            let new_count = app_state.redis_conn.hincrby(&key, &field, -1).await?;
            // Only positive counts are stored: a habit erased by failures is deleted, not
            // kept at zero (zeros would eat the combo cap; a raced re-creation via the
            // HGET->HINCRBY window lands <= 0 and is removed here too, best-effort).
            if new_count <= 0 {
                let _ = app_state.redis_conn.hdel_field(&key, &field).await;
            }
            Ok(StickyWriteOutcome::Decremented)
        }
        _ => Ok(StickyWriteOutcome::SkippedNoHabit),
    }
}
...
async fn prune_if_over_cap(key: &str, protect_field: &str) {
    ...
    // Cheap length gate first: the full HGETALL scan only runs when actually over cap.
    match app_state.redis_conn.hlen(key).await {
        Ok(len) if len as usize <= max_combos => return,
        Ok(_) => {}
        Err(err) => {
            logger::error!(
                action = "sticky_routing",
                "prune length check failed for {key}: {err}"
            );
            return;
        }
    }
...
mod tests {
    ...
    #[test]
    fn eroded_habits_never_pin() {
        // Failures floor a habit at zero; zero (or a raced-below-zero value) is no habit.
        let raw = HashMap::from([
            ("INTERAC:RTP:GIGADAT".to_string(), "0".to_string()),
            ("INTERAC:RTP:LOONIO".to_string(), "-1".to_string()),
            ("INTERAC:RTP:APEX".to_string(), "1".to_string()),
        ]);
        let data = StickyData::from_raw(raw);
        assert_eq!(
            data.connectors_for_combo("INTERAC", "RTP"),
            vec![("APEX".to_string(), 1)]
        );
    }
