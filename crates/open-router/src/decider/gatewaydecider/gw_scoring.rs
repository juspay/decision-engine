...
use rand::prelude::*;
use rand_distr::{Binomial, Beta, Distribution};
...
use crate::types::gateway_routing_input::{
    GatewayWiseSuccessRateBasedRoutingInput, SelectionLevel,
};
...
use crate::types::gateway_outage as ETGO;
...
use crate::types::order as Ord;
use crate::types::tenant_config as TenantConfig;
...
use super::types::{DebugScoringEntry, Gateway, GatewayRedisKeyMap, LogCurrScore, SrV3InputConfig};
...
pub fn sample_from_beta_distribution(
    ...
    let updated_gw_score = beta.sample(&mut rng);
    acc.insert(gw, updated_gw_score);
    acc
}
...
pub fn add_extra_score(
    ...
    pmt: String,
    pm: String,
    final_score_map: &mut GatewayScoreMap,
    gw: ETG::Gateway,
) -> &GatewayScoreMap {
    let gateway_sigma_factor = Utils::get_sr_v3_gateway_sigma_factor(merchant_sr_v3_input_config, &pmt, &pm, &gw)
        .or_else(|| Utils::get_sr_v3_gateway_sigma_factor(default_sr_v3_input_config, &pmt, &pm, &gw))
    ...
    let final_score = (score + extra_score).clamp(0.0, 1.0);
    final_score_map.insert(gw, final_score);
    final_score_map
}
...
pub async fn reset_sr_v3_score(
    ...
    let number_of_zeros = (((1.0 - max_score + lower_reset_factor * sigma) * float_bucket_size).floor() as i32).clamp(2, bucket_size);
    ...
    let key_score_map: Vec<_> = score_map
        .iter()
        .filter_map(|(gw, score)| {
            sr_gateway_redis_key_map.get(&format!("{:?}", gw)).map(|key| (key.clone(), *score))
        })
        .collect();
    let keys_for_reset: Vec<_> = key_score_map
        ...
        .map(|(key, _)| key.clone())
        .collect();
    let updated_score_map = score_map.iter().map(
        |(gw, score)| {
        (
            gw.clone(),
    ...
    keys_for_reset.iter().for_each(|key| {
        reset_gateway_for_sr_v3(score_reset_value, &score_list, key.clone()); // #TODO: Add await
    });
...
pub async fn get_gateway_score_based_on_sr_v3(
    bucket_size: i32,
    score_map: &mut GatewayScoreMap,
    gateway_redis_key: (ETG::Gateway, String),
) -> &GatewayScoreMap {
    let (gw, key) = gateway_redis_key;
    let score_key = format!("{}{}", key, "_}score");
...
pub fn create_score_map(score_map: &mut GatewayScoreMap, gw: ETG::Gateway) -> &GatewayScoreMap {
...
pub async fn reset_and_log_metrics(
    ...
    Utils::metric_tracker_log(
        ...
        "GW_SCORING",
        Utils::get_metric_log_format(decider_flow, metric_title.as_str()).await,
    )
    .await;
}
