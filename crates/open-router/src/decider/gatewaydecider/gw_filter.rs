...
pub async fn newGwFilters(
    ...
    if gws.is_empty() {
        ...
        let allMgas = if Utils::get_is_merchant_enabled_for_dynamic_mga_selection(this)
            .await { Utils::get_mgas(this) } else { None };
...
pub async fn getFunctionalGateways(this: &mut DeciderFlow<'_>) -> GatewayList {
    ...
    let enabled_gateway_accounts = if proceed_with_all_mgas {
        ...
        //     logDebugV(
        //         &format!(
        //             "metadata in getFunctionalGateways for txnId : {}",
        //             txn_id
        //         ),
        //         &meta,
        //     );
    ...
    let mgas_ = match (txn_detail.isEmi || Utils::is_reccuring_payment_transaction (&txn_detail), &enforce_gateway_list) {
    ...
    let edcc_mgas = if txn_detail.currency != oref.currency && is_edcc_applied == Some(true) {
        let edcc_supported_gateways: Vec<Gateway> = findByNameFromRedis(C::EDCC_SUPPORTED_GATEWAYS.get_key())
            .await
    ...
    let enforced_payment_flows: Vec<String> = filtered_mgas
        ...
        .flat_map(|mga| {
            mga.supported_payment_flows
                .as_ref()
                .and_then(|flows| flows.enforcedPaymentFlows.as_ref()).cloned()
                .unwrap_or_default()
        })
        .collect::<HashSet<_>>()
        .into_iter()
...
fn validateMga(
    ...
    } else { !Utils::is_only_subscription(mga) }
}
...
pub async fn filterFunctionalGateways(this: &mut DeciderFlow<'_>) -> GatewayList {
        ...
        if let Some(true) = mInternalMeta.as_ref().and_then(|meta| meta.isCvvLessTxn) {
            if txnCardInfo.authType == Some(AuthType::MOTO) {
                ...
                let authTypeRestrictedGateways =
                    findByNameFromRedis::<HashMap<AuthType, Vec<Gateway>>>(
                        C::AUTH_TYPE_RESTRICTED_GATEWAYS.get_key(),
                    )
                    .await
                    .unwrap_or_else(HashMap::new);
                let motoSupportedGateways: Vec<Gateway> = txnCardInfo
                    .authType
                    .as_ref()
                    .and_then(|auth_type| authTypeRestrictedGateways.get(auth_type))
                    .cloned()
                    .unwrap_or_default();
                let filtered_gateways: Vec<Gateway> = st
                    .into_iter()
                    .filter(|gw| motoSupportedGateways.contains(gw))
                    .collect();
                // log_info("filterFunctionalGateways", format!("Functional gateways after filtering for MOTO cvvLessTxns support for txn_id: {}", txnDetail.txnId));
                ...
                if isMerchantEnabledForCvvLessV2Flow {
                    let configResp = isPaymentFlowEnabledWithHierarchyCheck(
                        ...
                        TC::MERCHANT_CONFIG,
                        PF::CVVLESS,
                        crate::types::country::country_iso::text_db_to_country_iso(mAcc.country.as_deref().unwrap_or_default()).ok()
                    ).await;
                    let functionalGateways = if !configResp {
                        // log_error("CVVLESS-ERROR", "CVVLESS_FLOW_DISABLED");
                        Vec::new()
                    } else {
                        let st = getGws(this);
                        ...
                        if isBrandSupportsCvvlessTR {
                            ...
                            if let Some(cardPaymentMethod) = mPmEntryDB {
                                ...
                                let allGPMfEntries =
                                    ...
                                    .await;
                                let gmpfGws: Vec<Gateway> = allGPMfEntries
                                    .iter()
                                ...
                                let mgpmfEntries = MGPMF::get_all_mgpmf_by_mga_id_and_gpmf_ids(
                                    mgaIds, gpmfIds,
                                )
                                .await;
                                ...
                                let finalFilteredMga: Vec<MerchantGatewayAccount> = filteredMga
                                    .into_iter()
                                    .filter(|mga| {
                                        filteredMgaList.contains(&mga.id.merchantGwAccId)
                                    })
                                    .collect();
                    ...
                    // log_info("filterFunctionalGateways", format!("Functional gateways after filtering for token repeat cvvLessTxns support for txn_id: {}", txnDetail.txnId));
                ...
                let cardBrandToCvvLessTxnSupportedGateways: HashMap<String, Vec<Gateway>> =
                    findByNameFromRedis(
                        C::CARD_BRAND_TO_CVVLESS_TXN_SUPPORTED_GATEWAYS.get_key(),
                    )
                    .await
                    .unwrap_or_default();
                let cvvLessTxnSupportedCommonGateways: Vec<Gateway> =
                ...
                if !cvvLessTxnSupportedGateways.is_empty() {
                    ...
                    let filtered_gateways: Vec<Gateway> = st
                        ...
                        .collect();
                    // log_info("filterFunctionalGateways", format!("Functional gateways after filtering for cvvLessTxns for txn_id: {}", txnDetail.txnId));
                    setGws(this, filtered_gateways);
                }
            }
        }
    ...
    if Utils::is_card_transaction(&txnCardInfo) && txnCardInfo.authType == Some(AuthType::MOTO) {
        ...
        if paymentFlowList.contains(&"TA_FILE".to_string()) {
            let taOfflineEnabledGateways: Vec<Gateway> =
                findByNameFromRedis::<Vec<Gateway>>(C::TA_OFFLINE_ENABLED_GATEWAYS.get_key())
                    .await
                    .unwrap_or_default()
                    .into_iter()
                    .collect();
    ...
    let st = getGws(this);
    // log_debug("filterFunctionalGateways", format!("Functional gateways before filtering for MerchantContainer for txn_id: {}", txnDetail.txnId));
    let merchantContainerSupportedGateways: Vec<Gateway> =
        findByNameFromRedis(C::MERCHANT_CONTAINER_SUPPORTED_GATEWAYS.get_key())
            .await
            .unwrap_or_default();
    let filtered_gateways: Vec<Gateway> =
...
pub async fn filterByCardBrand(
    ...
    let amex_supported_gateways: HashSet<Gateway> =
    ...
    let amex_not_supported_gateways: HashSet<Gateway> =
    ...
    let sodexo_only_gateways: HashSet<Gateway> =
    ...
    let sodexo_also_gateways: HashSet<Gateway> =
...
pub async fn filterGatewaysForAuthType(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    if let Some(ref card_isin) = txn_card_info.card_isin {
        ...
        if mb_feature.is_none() {
            ...
            // debug!(
            //     "filterGatewaysForAuthType: Functional gateways after filtering after DISABLE_DECIDER_BIN_ELIGIBILITY_CHECK check: {}: {:?}",
            //     txn_detail.txnId,
            //     gci_validated_gws
            //         .iter()
...
pub async fn filterGatewaysForValidationType(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    if Utils::is_mandate_transaction(&txn_detail) && Utils::is_card_transaction(&txn_card_info) {
        ...
        // debug!(
        //     "filterGatewaysForValidationType: Functional gateways after filtering after filterGatewaysCardInfo for txn_id {}: {:?}",
        ...
        if Utils::is_token_repeat_txn(m_internal_meta.clone()) {
            ...
            // debug!(
            //     "filterGatewaysForValidationType: Functional gateways after filtering for token repeat Mandate support for txn_id {}: {:?}",
        ...
        if !txn_detail.expressCheckout && !Utils::is_token_repeat_txn(m_internal_meta) {
            let m_mandate_guest_checkout_supported_gateways: Option<Vec<ETG::Gateway>> =
                findByNameFromRedis(
            ...
            // debug!(
            //     "filterGatewaysForValidationType: Functional gateways after filtering for Mandate Guest Checkout support for txn_id {}: {:?}",
            //     txn_detail.txnId, final_gws
            // );
            ...
            && Utils::is_emandate_supported_payment_method(&txn_card_info))
    {
        ...
        if (validation_type == ETGCI::ValidationType::TpvEmandate
            ...
            && is_otm_flow
        {
            println!("Skipping processing for OTM flow");
        } else {
            ...
            // debug!(
            //     "filterGatewaysForValidationType: nst for filterGatewaysForValidationType for txn_id {}: {:?}",
            //     txn_detail.txnId, nst
            // );
        ...
        && (m_auth_type.is_some() || m_validation_type.is_some())
    {
            ...
            .unwrap_or(false)
        {
            ...
            let merchant_wise_eligible_gateway_card_info =
                if !merchant_wise_mandate_supported_gateway.is_empty() {
                    ETGCIS::getSupportedGatewayCardInfoForBins(
                        &appState,
                        merchant_account,
                        card_bins.clone(),
                    )
            ...
            let gcis_with_merchant_validation = gcis
                ...
                .filter(|gci| {
                    merchant_validation_required_gws
                        .contains(&gci.gateway.clone().unwrap_or(ETG::Gateway::NONE))
                })
                .cloned()
                .collect::<Vec<_>>();
            ...
            let mgcis_enabled_gcis = if gci_ids.is_empty() {
                ...
                ETMGCI::find_all_mgcis_by_macc_and_gci_p_id(
                    merchant_account.id,
                    gci_ids.clone(),
                )
                .await
        ...
        Some(txt) => {
            match serde_json::from_str::<GatewayRule>(txt) {
                ...
                Ok(gateway_rule) if gateway_rule.force_routing.unwrap_or(false) => {
                    ...
                    let txn_offer_details_gws: Vec<ETG::Gateway> = gateway_rule
                        .gateway_info
                        .iter()
                        .map(|info| info.name.clone())
                        .filter_map(|s| text_to_gateway(s.as_str()))
                        .collect();

                    // Create a HashSet from the offer gateways for efficient intersection
                    let offer_gw_set: HashSet<ETG::Gateway> =
                        txn_offer_details_gws.into_iter().collect();
...
pub async fn filterGatewaysForEmi(this: &mut DeciderFlow<'_>) -> GatewayList {
    ...
    if txn_detail.isEmi {
        let si_on_emi_card_supported_gateways: HashSet<Gateway> =
            findByNameFromRedis::<HashSet<Gateway>>(C::SI_ON_EMI_CARD_SUPPORTED_GATEWAYS.get_key())
                .await
                .unwrap_or_default()
                .into_iter()
                .collect();
        ...
        let gws = if Utils::check_no_or_low_cost_emi(&txn_card_info) {
            let no_or_low_cost_emi_supported_gateways: HashSet<Gateway> =
                findByNameFromRedis::<HashSet<Gateway>>(
        ...
        let gws = if !gws.is_empty() {
            let scope = if scope_ == "ALT_ID" { "CARD" } else { scope_ };
            // logDebugT(
            //     format!(
        ...
        let card_emi_explicit_gateways: HashSet<Gateway> =
            findByNameFromRedis::<HashSet<Gateway>>(C::CARD_EMI_EXPLICIT_GATEWAYS.get_key())
                .await
                .unwrap_or_default()
                .into_iter()
                .collect();
...
pub async fn filterGatewaysForPaymentMethod(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    if st.is_empty() || Utils::is_emandate_transaction(&txn) || Utils::is_tpv_transaction(&txn) || Utils::is_emandate_payment_transaction(&txn) {
        // logDebugV(
        ...
        let v2_integration_not_supported_gateways: Vec<Gateway> =
            findByNameFromRedis::<Vec<Gateway>>(
                C::V2_INTEGRATION_NOT_SUPPORTED_GATEWAYS.get_key(),
            )
            .await
            .unwrap_or_default()
            .into_iter()
        ...
        let upi_intent_not_supported_gateways: Vec<Gateway> =
            findByNameFromRedis::<Vec<Gateway>>(C::UPI_INTENT_NOT_SUPPORTED_GATEWAYS.get_key())
                .await
                .unwrap_or_default()
                .into_iter()
                .collect();
    ...
    match vault {
        ...
        Some(v) => {
            let token_provider_gateway_mapping =
                findByNameFromRedis::<HashMap<VaultProvider, Gateway>>(
...
pub async fn filterGatewaysForWallet(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    let st = getGws(this);
    let txn_card_info = this.get().dpTxnCardInfo.clone();
    let upi_only_gateways: HashSet<Gateway> = findByNameFromRedis::<HashSet<Gateway>>(C::UPI_ONLY_GATEWAYS.get_key())
        .await
        .unwrap_or_default()
        .into_iter()
        .collect();
    let wallet_only_gateways: HashSet<Gateway> =
        findByNameFromRedis::<HashSet<Gateway>>(C::WALLET_ONLY_GATEWAYS.get_key())
            .await
            .unwrap_or_default()
            .into_iter()
            .collect();

    let wallet_also_gateways: HashSet<Gateway> =
        findByNameFromRedis::<HashSet<Gateway>>(C::WALLET_ALSO_GATEWAYS.get_key())
            .await
            .unwrap_or_default()
            .into_iter()
            .collect();
...
pub async fn filterGatewaysForNbOnly(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    if txn_card_info.card_type != Some(ETCA::CardType::Nb) {
        let nb_only_gateways: Vec<Gateway> =
            findByNameFromRedis::<Vec<Gateway>>(C::NB_ONLY_GATEWAYS.get_key())
                .await
                .unwrap_or_default()
                .into_iter()
                .collect();
...
pub async fn filterGatewaysForConsumerFinance(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    let st = getGws(this);
    let txn_card_info = this.get().dpTxnCardInfo.clone();
    let consumer_finance_only_gateways: Vec<Gateway> =
        findByNameFromRedis::<Vec<Gateway>>(C::CONSUMER_FINANCE_ONLY_GATEWAYS.get_key())
            .await
            .unwrap_or_default()
            .into_iter()
            .collect();
    ...
    if txn_card_info.paymentMethodType == ETP::PaymentMethodType::ConsumerFinance {
        let consumer_finance_also_gateways: Vec<Gateway> =
            findByNameFromRedis::<Vec<Gateway>>(C::CONSUMER_FINANCE_ALSO_GATEWAYS.get_key())
                .await
                .unwrap_or_default()
                .into_iter()
                .collect();
...
pub async fn filterGatewaysForUpi(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    let txn_card_info = this.get().dpTxnCardInfo.clone();
    let txn_detail = this.get().dpTxnDetail.clone();
    let upi_only_gateways: Vec<Gateway> =
        findByNameFromRedis::<Vec<Gateway>>(C::UPI_ONLY_GATEWAYS.get_key())
            .await
            .unwrap_or_default()
            .into_iter()
            .collect();
    ...
    if txn_card_info.paymentMethodType == ETP::PaymentMethodType::UPI {
        let upi_also_gateway: Vec<Gateway> =
            findByNameFromRedis::<Vec<Gateway>>(C::UPI_ALSO_GATEWAYS.get_key())
                .await
                .unwrap_or_default()
                .into_iter()
                .collect();
...
pub async fn filterGatewaysForTxnType(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    match maybe_txn_type {
        ...
        Some(txn_type) => {
            ...
            let txn_type_gateway_mapping =
                findByNameFromRedis::<HashMap<String, Vec<Gateway>>>(C::TXN_TYPE_GATEWAY_MAPPING.get_key())
                    .await
                    .unwrap_or_default();
            setGwsAndMgas(
                this,
...
fn getTxnTypeSupportedGateways(
    txn_type: &str,
    txn_type_gateway_mapping: &HashMap<String, Vec<Gateway>>,
) -> Vec<Gateway> {
...
fn get_zero_auth_supported_gateways(
    txn_type: &str,
    txn_detail_type_restricted_gateways: &Vec<(String, Vec<Gateway>)>,
) -> Vec<Gateway> {
...
pub async fn filterGatewaysForReward(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    let reward_also_gateways: HashSet<Gateway> =
        findByNameFromRedis(C::REWARD_ALSO_GATEWAYS.get_key())
            .await
            .unwrap_or_else(Vec::new)
            .into_iter()
            .collect();
    let reward_only_gateways: HashSet<Gateway> =
...
pub async fn filterGatewaysForCash(this: &mut DeciderFlow<'_>) -> Vec<ETG::Gateway> {
    ...
    if payment_method_type != ETP::PaymentMethodType::Cash {
        let cash_only_gateways: Vec<ETG::Gateway> =
            findByNameFromRedis(C::CASH_ONLY_GATEWAYS.get_key())
                .await
                .unwrap_or_else(Vec::new)
                .into_iter()
                .collect();
...
pub async fn filterFunctionalGatewaysForSplitSettlement(
    this: &mut DeciderFlow<'_>,
) -> Vec<ETG::Gateway> {
    let oref = this.get().dpOrder.clone();
    ...
    match e_split_settlement_details {
        ...
        Err(msg) => {
            ...
            // );
            let st = getGws(this);
            let split_settlement_supported_gateways: Option<Vec<ETG::Gateway>> =
