        ...
        Some(key) => {
            ...
            let gw_score_to_be_updated = match m_cached_gateway_score {
                ...
                Some(cached_gateway_score) => {
                    if (timestamp - cached_gateway_score.timestamp as u128) > (hard_key_ttl as u128) - 1000 {
            ...
            let remaining_ttl = hard_key_ttl as u128- (timestamp - updated_cached_gateway_score.timestamp as u128).max(0);
            ...
            //To Do: add Ok & Err
            // match EulerTransforms::writeToCacheWithTTL(key, updated_cached_gateway_score, safe_remaining_ttl).await;{
...
pub fn getNewCachedGatewayScore(
    ...
    if isGlobalKey(score_key_type) {
        ...
        CachedGatewayScore {
            score: Some(1.0),
            timestamp: current_date as i32,
            lastResetTimestamp: Some(current_date as i32),
            merchants: None,
...
pub async fn getAllUnifiedKeys(
    ...
    let is_key_enabled_for_merchant_gateway_scoring = MCU::isPaymentFlowEnabledWithHierarchyCheck(mer_acc_p_id, mer_acc.tenantAccountId.clone(), ModuleEnum::MERCHANT_CONFIG, PaymentFlow::PaymentFlow::ELIMINATION_BASED_ROUTING, 
...
pub async fn getTTLForKey(score_key_type: ScoreKeyType) -> i32 {
...
fn getDefaultTTL(score_key_type: ScoreKeyType) -> i32 {
    if isGlobalKey(score_key_type) {
