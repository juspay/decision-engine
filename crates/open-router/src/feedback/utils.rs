...
use crate::feedback::types::{CachedGatewayScore,MandateTxnType,MandateTxnInfo,UpdateScorePayload,InternalMetadata,InternalTrackingInfo};
...
pub fn getTxnDetailFromApiPayload(apiPayload: UpdateScorePayload, gateway_scoring_data: GatewayScoringData) -> ETTD::TxnDetail{
    let txn_detail = ETTD::TxnDetail {
        ...
        txnObjectType: ETTD::TxnObjectType::from_text(gateway_scoring_data.orderType.clone()).unwrap_or_else(|| ETTD::TxnObjectType::OrderPayment),
        sourceObject: Some(gateway_scoring_data.paymentMethod.clone()),
        sourceObjectId: None,
        currency: Currency::INR,
        surchargeAmount: None,
        taxAmount: None,
        internalMetadata: Some(serde_json::to_string(&InternalMetadata { internal_tracking_info: InternalTrackingInfo { routing_approach: gateway_scoring_data.routingApproach.unwrap_or_default() } }).unwrap()),
...
pub fn getTxnCardInfoFromApiPayload (apiPayload: UpdateScorePayload, gateway_scoring_data: GatewayScoringData) -> ETCa::txn_card_info::TxnCardInfo {
    let txnCardInfo =  ETCa::txn_card_info::TxnCardInfo {
        ...
        nameOnCard: None,
        dateCreated: gateway_scoring_data.dateCreated,
        paymentMethodType: ETP::PaymentMethodType::from_text(&gateway_scoring_data.paymentMethodType.clone()).unwrap_or(ETP::PaymentMethodType::Unknown),
        paymentMethod: gateway_scoring_data.paymentMethod.clone(),
        paymentSource: gateway_scoring_data.paymentSource.clone(),
        authType: gateway_scoring_data.authType.clone().and_then(|auth_type_text| {
...
pub async fn updateScore(redis:String, key: String, should_score_increase: bool) -> () {
    ...
    match either_res {
        Ok(_int_val) => (),
        Err(err) => {
            println!("Error while updating score in redis: {}", err);
        }
    }
}
...
pub async fn isKeyExistsRedis(key: String) -> bool {
    let app_state = get_tenant_app_state().await;
    let either_is_in_redis = app_state.redis_conn.get_key(&key, "String").await;
    match either_is_in_redis {
        Ok(val) => val,
        Err(err) => {
            println!("Error while checking key in redis: {}", err);
            false
        }
    }
}
...
pub async fn updateQueue(
    ...
    match r {
        Ok(result) => {
            println!("Successfully updated queue in Redis: {:?}", result);
            Ok(result)
        }
        Err(e) => {
            println!("Error while updating queue in Redis: {:?}", e);
            Ok(vec![value_clone])
        }
    }
}
...
pub async fn getProducerKey(
    ...
    match redis_gateway_score_data {
        Some(gateway_score_data) => {
            ...
            let gateway = txn_detail
                .gateway
                .and_then(|gateway| gateway_any_to_text(&GatewayAny::GatewayKnown(gateway)))
                .unwrap_or_default();
        ...
        None => {
            println!("Gateway score data is None");
            None
        }
    }
}
...
pub async fn writeToCacheWithTTL(
    ...
    let primary_delete = deleteFromCache("kv_redis".to_string(), key).await;
...
pub async fn delCache(dbName: String, key: String) -> Result<i32, StorageError> {
    ...
    match data {
        ...
        Err(err) => {
            println!("Error while deleting score key in redis: {}", err);
